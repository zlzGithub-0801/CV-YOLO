...
# ---------- 1. better_images 平均 score & 平均提升 ----------
better_scores = []
better_deltas = []

for name in better_images:
    if name in output_map and name in baseline_map:
        better_scores.append(output_map[name])
        better_deltas.append(output_map[name] - baseline_map[name])

if better_scores:
    avg_better_score = sum(better_scores) / len(better_scores)
    avg_better_delta = sum(better_deltas) / len(better_deltas)
else:
...
for name in selected_list[:30]:
    ...
    if name in baseline_map:
        selected_baseline_scores.append(baseline_map[name])

avg_selected_output = (
    sum(selected_output_scores) / len(selected_output_scores)
    if selected_output_scores else 0.0
)

avg_selected_baseline = (
    sum(selected_baseline_scores) / len(selected_baseline_scores)
...
print(f"平均提升 (output - baseline): "
      f"{avg_selected_output - avg_selected_baseline:+.4f}")
